...
impl ScriptEditor {
    ...
    pub fn compile_check(&mut self) {
        ...
        if !diagnostics.has_error() && diagnostics.is_empty() {
            self.diagnostics = Vec::new();
            return;
        }
        ...
        // If we got no structured output, add a generic entry
        if diags.is_empty() {
...
pub fn render(ui: &mut egui::Ui, editor: &mut ScriptEditor, save_requested: bool) {
    ...
    let diag_height = if editor.diagnostics.is_empty() { 0.0_f32 } else { 120.0 };
    let row_height  = fontsize * 1.4; // approximate line height for this font size
    let available_h = (ui.available_height() - diag_height - 4.0).max(80.0);
    let rows        = (available_h / row_height).floor().max(4.0) as usize;
